...
class JREProcessor:
    def __init__(self):
        ...
        # Load configuration
        config = Config()
        self.platform_tags = config.get_platform_tags()
    ...
    def fetch_and_update_data(self):
        ...
        self.combined_df['green_jres'] = self.combined_df['jre_version'].apply(lambda x: x if x and (x.startswith('17') or x.startswith('21') or x.startswith('1.8.0') and int(x.split('_')[1]) >= 352) else None)
        ...
        self.combined_df['env_var_cmd'] = self.combined_df['jre_version'].apply(lambda x: x if x and 'JAVA 8' in x else None)
