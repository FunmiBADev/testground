...
def save_daily_counts():
    ...
    if not os.path.isfile('daily_jre_counts.csv'):
        df.to_csv('daily_jre_counts.csv', index=False)
    else:
        df.to_csv('daily_jre_counts.csv', mode='a', header=False, index=False)
...
# Aggregate data for all platform tags
aggregated_data = combined_df.groupby('platform_tag').agg(
    green_jres_count=pd.NamedAgg(column='green_jres', aggfunc=lambda x: x.notna().sum()),
    eol_jres_count=pd.NamedAgg(column='eol_jres', aggfunc=lambda x: x.notna().sum()),
    env_var_cmd_count=pd.NamedAgg(column='env_var_cmd', aggfunc=lambda x: x.notna().sum())
).reset_index()

# Create a stacked bar chart
bar_chart_df = pd.melt(aggregated_data, id_vars=['platform_tag'], 
